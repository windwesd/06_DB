...
/* ***** LIKE(같은, 비슷한) ***** 
 * 
 * - 비교하려는 값이 특정한 패턴을 만족 시키면 조회하는 연산장
 * 
 * [작성법]
 * 
 * WHERE 컬럼명 LIKE '패턴'
...
-- EMPLOYEE 테이블에서 이름이 '하' 가 포함된 사원의 사번, 이름 조회
	SELECT EMP_ID, EMP_NAME 
	FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 EMAIL의 아이디의 글자 수가 5글자인 사원의 사번, 이름, 이메일 조회
...
-- EMPLOYEE 테이블에서 EMAIL의 아이디 중 '_' 앞쪽 글자 수가 3글자인 사원의 사번, 이름, 이메일 조회
  ...
  /*
  	...
  	- 작성되는 문자열의 '_' 기호가 LIKE '_' 와일드 카드와 똑같이 인식되서 구분이 안되는 문제가 발생!!!
...
-- 오름차순(ASCENDING) : 갈수록 커지는 순서
--  EX) 1 ~ 10 / 가 -> 하 / A -> Z / 과거 -> 미래
  
-- 내림차순(DESCENDING) : 점점 작아지는 순서
  
-- EMPLOYEE 테이블에 존재하는 모든 사원을 이름 오름차순으로 조회
  SELECT EMP_NAME 
  FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 부서코드가 'D5, 'D6', 'D9' 인 사원의 사번, 이름 부서코드를
-- 부서코드 오름차순으로 조회
 	SELECT EMP_ID, EMP_NAME, DEPT_CODE
 	FROM EMPLOYEE
 ...
 /* 컬럼 순서를 이용해서 정렬 */
-- 급여가 400만 이상, 600만 이사인 사원의 사번, 이름, 급여를 급여 내림차순으로 조회
 SELECT EMP_ID, EMP_NAME, SALARY 
 FROM EMPLOYEE
...
/* 별칭을 이용해서 정렬 */
-- 직급 코드가 'J4', 'J5', 'J6' 사원을 사번, 이름, 직급코드 조회 하기 단, 이름 오름차순으로 조회
...
--> WHERE절은 SELECT절보다 먼저 해석되는데 별칭은 SELECTㅈ러이 해석 되어야 등록되기 때문에
--  WHERE절에서는 별칭 사용 불가능!!

/* ORDER BY 절에 수식 적용 */
-- 모든 사원의 이름, 연봉을 연봉 내림차순으로 조회
	SELECT EMP_NAME, SALARY*12 
	FROM EMPLOYEE
...
-- 모든 사원 사번, 이름, 부서코드를 부서코드 오름 차순으로 조회
	SELECT EMP_ID, EMP_NAME /*, DEPT_CODE*/ 
	FROM EMPLOYEE
	ORDER BY DEPT_CODE ASC;

--> SELECT 절에 작성되지 않아도 이전에 FROM절이 해석되었기 때문에 정렬 가능!!

/* NULLS FIRST / JULLS LAST 확인 */
