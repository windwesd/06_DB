...
-- 부서코드 순으로 정렬
SELECT DEPT_CODE, COUNT(*) 	-- 4.조회 결과에 포함될 컬럼을 지정
 ...
 * 
 * HAVING	: GROUP BY를 통해 만들어진 그룹 증 조건에 맞는 그룹만 골라내는 구문
 * */
...
ORDER BY DEPT_CODE ASC;
--> EMPLOYEE 테이블에서 DEPT_CODE 기준으로 만들어진 그룹 중 그룹별 급여 평균이
--  350만 이상인 그룹만 조회 (맞는 SQL)
...
--> 교집합인 심봉선, 대북혼을 제외한 앞쪽 SELECT의 RESULT SET이 조회됨
SELECT EMP_NAME, DEPT_CODE, SALARY 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
