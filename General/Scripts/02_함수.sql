...
SELECT MONTHS_BETWEEN( TO_DATE('2024-09-16', 'YYYY-MM-DD'),
												CURRENT_DATE) 
FROM DUAL;
...
/* MONTH_BETWEEN이 지정된 두 날짜 사이의 차를 계살할 때
 * 훨씬 좋다!!(더 정확함)
...
--> 지정된 포맷의 칼 수가 변환하려는 숫자 자릿수 보다 적은 경우
-- 모든 숫자가 #으로 변환되서 출력
...
--> [해결방법] : ""로 감싸서 패턴을 나타내는 기호가 아닌 있는 그대로 출력하는 글자임을 명시
...
SELECT TO_DATE('24년 08월 16일 금요일 오후 2시 15분 30',
...
-- 50 미만 확인
SELECT TO_DATE('49-12-25', 'YY-MM-DD'),
			 TO_DATE('49-12-25', 'RR-MM-DD') 
FROM DUAL;

-- 50 이상 확인
SELECT TO_DATE('50-12-25', 'YY-MM-DD'),
...
-- 전체 조회 결과 행 중에서 PHONE 컬럼에 값이 작성된 행만 개수 카운트해서 반환
SELECT COUNT(PHONE) 
FROM EMPLOYEE;
