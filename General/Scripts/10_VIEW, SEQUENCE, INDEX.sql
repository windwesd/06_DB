...
-- V-DCOPY2 VIEW를 이용해서 INSERT 수행
INSERT INTO V_DCOPY2
VALUES ('D0', 'L2');
-- 가상의 테이블 VIEW인데 INSERT가 성공합!!

-- INSERT 결과 확인 - V_DCOPY2 VIEW
SELECT * FROM V_DCOPY2;
-- D0		L2

-- INSERT 결과 확인 - V_DCOPY2 원본 테이블
SELECT * FROM DEPT_COPY2;
-- D0		NULL		L2

-- VIEW에 INSERT를 수행 했지만 VIEW를 만들 때 사용한 원본 테이블에 값이 INSERT됨을 확인!!

--> 하지만 모든 컬럼 값이 INSERT 된것이 아니라, VIEW를 생성할 때 사용된 컬럼에만 값이 INSERT 되어
...
--> 발생 원인	: CURRVAL의 정확한 의미는 가장 최근 호출된 NEXTVAL의 값을 반환함
	--> NEXTVAL를 호출한 적이 없어서 오류 발생!!

--> 해결 방법 : NEXTVAL를 호출하면 해결
SELECT SEQ_TEST_NO.NEXTVAL
FROM DUAL; -- 시퀀스 생성 후 첫 NEXTVAL == START WITH 값인 100

-- NEXTVAL를 호출할 때 마다 INCREMENT BY에 작성된 수 만큼 증가
SELECT SEQ_TEST_NO.NEXTVAL
FROM DUAL; 
-- 처음 : 100
-- 1회 : 105
...
WHERE TEST_ID = 'TEST500000'; -- 0.014
