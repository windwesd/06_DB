...
--> 계정 생성이 성공하면 왼쪽 Database Navigator에서 TEST_USER 접속 방법 추가해서 연결 테스트 해보기
...
/* TEST_USER 계정에는 
 * 데이터 베이스 접속 권한은 CREAT SESSION만 부여됨
 * 
...
-- TEST_USER 계정이 생성한 객체는 USERS 폴더(스토리지)에 만들어지고
-- 사용 가능한 용량은 10M로 제한
...
SELECT * FROM KH_KHJ.EMPLOYEE;

-- 다른 테이블 조회도 되는지 확인
SELECT * FROM KH_KHJ.DEPARTMENT;
...
SELECT * FROM KH_KHJ.EMPLOYEE; -- 조회 불가
...
-- ORA-01940: 현재 접속되어 있는 사용자는 삭제할 수 없습니다
 --> Database Navigator 에서 TEST_USER 계정에 접속 중이라서 삭제 불가
 --> 연결 종료
