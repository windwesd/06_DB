...
<body>
   ...
   * src/main/webapp
    - 실제로 배포되는 웹 어플리케이션 컴파일된 코드, 정적 자원(html, css, js, image)가 모여있는 폴더
   ...
   * src/main/webapp/WEB-INF 폴더
    ...
    - 이럴꺼면 배포하는 webapp폴더에 안넣으면 되는거 아닌가?
      -> 직접 해당 파일을 보여줄 순 없지만 응답 화면 중에 웹 서비스를 제공하기 위해서는 필요하기 때문에 webapp 폴더에 존재해야 한다
  </pre>
  ...
  <pre>
    ...
    - Servlet에 존재하는 이름이 모인 List에서 입력 받은 이름과 같은 이름이 있는지 확인

    - 존재하는 경우 -> "OOO님은 X번째 인덱스에 존재 합니다" 문자열을 request scope 객체에 세팅
                        /WEB-INF/views/result.jsp로 요청 위임해 결과 출력 (forward)
    
    - 존재하지 않는 경우 -> "OOO님은 존재하지 않습니다" 문자열을 sessiong scope에 세팅 후 
