...
public class PracticeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// int List.indexOf("값") 값과 일치하는 요소의 index를 반환 없으면 -1 반환
		int index = nameList.indexOf(inputName);
		
		// 존재하는 경우 - forward
		if(index > -1){
			String message = String.format("%s님은 %d번째 인덱스에 존재 합니다", inputName, index);
			...
			// -> 지정된 결로의 jsp에서 응답을 대신하라고 위임
			// 	이 때, 기존 req/resp가 유지됨!
			//			응답화면의 주소 : /serach (현재 servlet 매핑 주소)
		}
		...
		// 존재하지 않은 경우
			...
			//	 기존 req 값을 세ㅣㅇ해봐야 없어지기 때문에 의미 없음
